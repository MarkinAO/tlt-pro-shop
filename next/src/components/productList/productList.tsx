"use client";
import { GetProducts } from "@/api/hooksAPI";
import { useStore } from "@/model/store";
import { useEffect } from "react";
import Image from "next/image";

export const ProductList = () => {
  const { listProd, setProducts } = useStore((state) => state);
    ...
    <>
      ...
      <div className="flex flex-col gap-[10px]">
        {listProd.map((el, i) => {
          return (
            <div
              className={`flex justify-between h-[80px] px-[10px] items-center ${
                i % 2 && "bg-[#0F172A]/[.03] rounded-[6px]"
              }`}
              key={i}
            >
              <div className="w-1/6">
              ...
              <div className="w-1/6 text-center">{el.name}</div>
              <div className="w-1/6 text-center">{el.quantity}</div>
              <div className="w-1/6 text-center">{el.manufacturerId}</div>
              <div className="w-1/6 text-center">{el.price} р</div>
